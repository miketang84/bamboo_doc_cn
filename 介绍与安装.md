...
编辑配置文件 /etc/redis.conf，如果没有此文件就把当前redis源代码包中的redis.conf文件拷贝到/etc/下面去。配置文件可根据实际需求做更改。现将我的配置文件粘贴如下，供参考：
...
将以下内容拷贝到此文件中去 
	...
	static_apptest = { type="dir", base='sites/apptest/', index_file='index.html', default_ctype='' }
	handler_apptest = { type="handler", send_spec='tcp://127.0.0.1:10001',
